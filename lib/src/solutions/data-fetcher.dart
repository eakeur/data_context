...
import 'package:datacontext/src/solutions/data-awaiter.dart';
...
class DataFetcher {
  ...
  DataFetcher(this.path, {this.headers = const <String, String>{}, this.onSending, this.onReceiving}) : client = Client();
  ...
  Future<DataAwaiter<T>> add<T extends DataClass>(String route, T data) async {
    ...
    if (onSending != null) onSending!(uri, headers, null, DataOperation.ADD); var body;
    try {body = data.toJson();} catch (e) {throw ArgumentError('Failed converting data to JSON: ${data.toString()}');}
  ...
  Future<DataAwaiter<T>> update<T extends DataClass>(String route, T data) async {
    var uri = mountUri(route, {});
    if (onSending != null) onSending!(uri, headers, null, DataOperation.UPDATE); var body;
    try {body = data.toJson();} catch (e) {throw ArgumentError('Failed converting data to JSON: ${data.toString()}');}
    var response = await client.post(uri, body: body as String, headers: headers);
    if (onReceiving != null) onReceiving!(response);
    return DataAwaiter<T>(response.body);
  ...
  Future<DataAwaiter<T>> remove<T extends DataClass>(String route, {T? data, Map<String, dynamic> query = const <String, String>{}}) async {
    var uri = mountUri(route, query);
    if (onSending != null) onSending!(uri, headers, query, DataOperation.REMOVE); var body;
    try {if (data != null) body = data.toJson();} catch (e) {throw ArgumentError('Failed converting data to JSON: ${data.toString()}');}
  ...
  Uri mountUri(String route, Map<String, dynamic> query) {
    ...
    var url = members.reduce((val, el) => val + (val.endsWith('/') ? el : '/$el'));
